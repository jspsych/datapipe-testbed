...
> **Where the source paths point.** Paths like `components/dashboard/…`,
> `pages/admin/…`, `lib/…` and `functions/src/…` are in the DataPipe repo
> (`jspsych/datapipe`), not this one. Read the deployed version with
> `git -C <your datapipe checkout> show origin/test:<path>` (after a
> `git -C … fetch origin test`), or on GitHub. The test deployment is built
> from that repo's `test` branch and production from `main`.
