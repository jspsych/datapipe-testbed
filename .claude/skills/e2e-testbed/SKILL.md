...
<https://jspsych.github.io/datapipe-testbed/> (source: `site/` in THIS repo) that send
real data to a real DataPipe experiment. Every run publishes a machine-readable
...
Fetch `https://jspsych.github.io/datapipe-testbed/scenarios.json` — the PUBLISHED copy
describes the pages you will actually load. Compare it with `site/scenarios.json` in this
checkout: if they differ, a change is unmerged or unpublished, so use the published one
and say so. If it is not published at all, fall back to the prose "What to check" list on
the home page (result contract absent too — see §5 — every scenario is `automation: agent`).
...
a run will trip over" in [docs/e2e-testing.md](../../../docs/e2e-testing.md)** (the manifest's `knownIssues` says the
same) — several things there look like bugs and are not.

## 10. Report

Write [output-template.md](output-template.md), filled in, to
`runs/<YYYY-MM-DD>-<topic>.md` in this repo and commit it. This repo is PUBLIC, so
redact as `runs/README.md` says: no account emails, no storage folder IDs or links, no
tokens or session IDs, no local paths. Experiment IDs are public by design.
