...
**The testbed** — <https://jspsych.github.io/datapipe-testbed/>, source in
`site/` here. It tests [DataPipe](https://github.com/jspsych/datapipe), whose
code lives in its own repository; this one holds everything about testing it
end to end. Two
...
**The scenario manifest** — `site/scenarios.json` in this repo is the single
...
the Drive folder, probe the endpoints directly, and write a report to `runs/`
in this repo (committed; this repo is public, so reports are redacted as
`runs/README.md` says). Start the session from a checkout of THIS repo so the
skill loads: `cd datapipe-testbed && claude`, then `/e2e-testbed`. The reference files
...
1. A long-lived e2e experiment on `datapipe-test` with data collection,
   ...
   experiment has data.
2. A Playwright workflow in this repo, started by a `repository_dispatch` that
   the DataPipe repo's **Deploy to Test** workflow sends when it succeeds (a
   `workflow_run` trigger cannot cross repositories), reading `scenarios.json`
   from the published testbed.
